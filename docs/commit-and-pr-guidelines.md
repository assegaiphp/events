...
Use this format for `assegaiphp/events` commits and pull requests.
...
```text
fix(events): preserve listener failure hooks
feature(events): add package installer wiring
test(events): cover wildcard listener dispatch
docs(events): clarify outbox usage
```
